# Contribute to the Bifröst Front-end Library
...
The Bifröst Design-system is the combination of spaces
...
-   **JS Scripts** : Javascript required for Bifröst components.
...
## Run Bifröst Library (Storybook)

Ask Bifröst Team for access to the Bifröst repository then start it:
...
⚠️ This showcase is in progress. It's possible that you will not be able to launch Drupal.
...
-   Review visual tests with Chromatic (🚧 work in progress).
...
> _Don't forget to use hyphens here instead of dots_
...
## 3) Commit, Push & create a Pull Request on GitHub
...
2. In [Jira Kanban view](https://groupemedia.atlassian.net/jira/software/c/projects/DS/boards/1036), for all the tickets that are in the `READY TO PACKAGE` column, if it's not already done, add a `Fix versions / Versions corrigées` to the same number as the new Jira release number.
...
```markdown
### Storybook changelog
https://bifrost-front-end-library-git-v1-11-0-bifrost-vui.vercel.app/?path=/docs/changelog--page

### ChangeLog Jira
https://groupemedia.atlassian.net/projects/DS/versions/38177/tab/release-report-all-issues

### NPM package
https://www.npmjs.com/package/@videotron-ui/bifrost-front-end-library/v/1.11.0
```
