import { moduleMetadata } from '@storybook/angular';
import { Canvas, Meta, Story, Props } from '@storybook/addon-docs';

import { topBar_items, topBar_secondaryItems, rigthIconLinks, cartBtn, menuItemAccount, loginMenuItems, menuItemAccountNotConnected } from './data';
import { TwigContainerComponent }   from '../../../../utils/twig-container/twig-container.component';
import { HeaderComponent }          from '../angular/header.component';


<Meta
    title='Components/Navigation Header'
    component={HeaderComponent}
    decorators={[
        moduleMetadata({ declarations: [HeaderComponent, TwigContainerComponent]})
    ]}
    argTypes={{
        accountHello: {
            table: { defaultValue: { summary: 'Hello' } },
        },
        langcode: {
            defaultValue: 'FR',
            control: {
                type: 'select',
                options: ['EN', 'FR'],
            }
        },
        hasTopBar: {
            defaultValue: true,
        },
        reversed: {
            defaultValue: false,
        }
    }}
/>


# Header

<br/>

## Component API

<Props of={HeaderComponent} />


## Drupal implementation example

```jsx
{% include '@bf-components/navigation-header/twig/index.twig' with {
    hasTopBar: true,
    topBar: {
        items: [
            { label: 'Personal',    href: '', class: 'active',  },
            { label: 'Business',    href: '', class: '' },
        ],
        secondaryItems: [
            { label: 'Webmail',     href: '', class: '' },
            { label: 'illico.tv',   href: '', class: '' },
            { label: 'Locate',      href: '', class: '' }
        ],
    },
    logo : {
        name : 'business',
    },
    navItems: [
        {
            label: 'Shop', href: '', class: '',
            subItemsMultiDropdown: [
                {
                    label : 'Helix plans', href : '', class : '',
                    listItems: [
                        { label : 'The Helix experience',   href : '', class : '' },
                        { label : 'Helix TV',               href : '', class : '' },
                        { label : 'Helix Internet',         href : '', class : '' },
                    ]
                },
                {
                    label : 'Other custom bundles', href : '', class : '',
                    listItems: []
                },
            ],
            mega: [
                { imgSrc: 'https://source.unsplash.com/400x250/?nature', imgAlt: 'Thumbnail', href: '#', title: '', text: '' },
                { imgSrc: 'https://source.unsplash.com/400x250/?nature', imgAlt: 'Thumbnail', href: '#', title: '', text: '' },
            ]
        },
        {
            label : 'Support', href : '#', class : '',
            subItemsMultiDropdown: [...],
            mega: [...],
        }
        ...
    ],
 navMobileItems: [
        {
            label: 'Shop', href: '', class: '',
            subItemsMultiDropdown: [
                {
                    label : 'Helix plans', href : '', class : '',
                    listItems: [
                        { label : 'The Helix experience',   href : '', class : '' },
                        { label : 'Helix TV',               href : '', class : '' },
                        { label : 'Helix Internet',         href : '', class : '' },
                    ]
                },
                {
                    label : 'Other custom bundles', href : '', class : '',
                    listItems: []
                },
            ],
            mega: [
                { imgSrc: 'https://source.unsplash.com/400x250/?nature', imgAlt: 'Thumbnail', href: '#', title: '', text: '' },
                { imgSrc: 'https://source.unsplash.com/400x250/?nature', imgAlt: 'Thumbnail', href: '#', title: '', text: '' },
            ]
        },
        {
            label : 'Support', href : '#', class : '',
            subItemsMultiDropdown: [...],
            mega: [...],
        }
        ...
    ],
    rigthIconLinks : [
        { icon : 'email',   href : ' ', class : '' },
        { icon : 'pin',     href : ' ', class : '' },
    ],

    hasCartBtn : true,
    cartBtn : {
        badgeLabel       : '3',
        href        : 'https://www.videotron.com',
        ariaLabel   : 'My cart',
        class       : ''
    },
    menuItemAccount : [
        { label : 'Action', href: '#' },
        { label : 'Another action', href: '#' },
        { label : 'Something else here', href: '#' },
    ],
    hasAccountBtn   : true,
    isConnected     : true,
    hrefLogOut      : '#',
    labelLogOut     : 'Log out',
    labelLogIn      : 'Log In',
    loginMenuItems  : {
        name        :'John Doe',
        description :'Bill and customer center',
        href: "https://www.videotron.com"
    },
    menuItemAccountNotConnected : [
        { label:"Webmail", href:"#", subIconName:"email" },
        { label:"Helix", href:"#", subIconName:"helix-tv" },
        { label:"illico", href:"#", subIconName:"tv-plan-light" },
    ];
    hrefConnexion   : '#',
    labelConnexion  : 'Connexion',

    search          : {
        placeholder : 'What are you looking for?'
    },

    reversed : true,
    langcode : 'EN'
} only %}
```
## Drupal

export const Drupal = (args) => ({
    component: TwigContainerComponent,
    props: args,
});

<Canvas withSource='none'>
    <Story
        name='Drupal'
        args={{
            elementPath : 'components/navigation-header/_doc/template.drupal',
            topBar_items,
            topBar_secondaryItems,
            logo : {
                name                : 'business',
                href                : '/',
            },
            rigthIconLinks,
            menuItemAccount,
            cartBtn,
            loginMenuItems,
            menuItemAccountNotConnected
        }}
        height='400px'
        with = '1200px'
    >
        {Drupal.bind({})}
    </Story>
</Canvas>

## more info