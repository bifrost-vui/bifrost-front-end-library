import { moduleMetadata } from '@storybook/angular';
import {  ArgsTable, Canvas, Meta, Story } from '@storybook/addon-docs';
import { withDesign } from 'storybook-addon-designs'
import { TwigContainerComponent } from '../../../../utils/twig-container/twig-container.component';
import { BannerAdvancedComponent } from '../angular/banner-advanced.component';

<Meta
    title='Components/Banner Advanced'
    component={BannerAdvancedComponent}
    decorators={[
        moduleMetadata({ declarations: [BannerAdvancedComponent, TwigContainerComponent]}),
        withDesign
    ]}
    parameters={{
        design: {
            type: 'figma',
            url: 'https://www.figma.com/file/7bTmk29aCBl7rCFszffYPr/DS---Components-%26-styles?node-id=21784%3A284582',
            disable: false
        },
    }}
    argTypes={{}}
/>

# Banner Advanced

## Component API

<ArgsTable of={BannerAdvancedComponent} />


## Drupal

export const Drupal = (args) => ({
    component: TwigContainerComponent,
    props: args,
});

export const commonProps= {
    title: 'Mobile discount of $10 with an Internet plan',
    description: 'Combine a Mobile plan to an Internet plan and get a $10 monthly discount.',
    icons: [
        {
            name: 'mobility',
            label: 'Mobile plan'
        },
        {
            name: 'bundle-1',
            label: 'Internet plan'
        }
    ]
}

<Canvas
    withSource='none'
    style={{height: '450px'}}
>
    <Story
        name='Drupal'
        args={{
            elementPath: 'components/banner-advanced/_doc/template.drupal',
            ...commonProps
        }}
        >
        {Drupal.bind({})}
    </Story>
</Canvas>

<Canvas
    withSource='none'
    style={{height: '600px'}}
>
    <Story
        name='Drupal - with footer'
        args={{
            elementPath: 'components/banner-advanced/_doc/template.drupal',
            inputSwitch: {
                id: 'inputId',
                name: 'inputName',
                ariaLabel: 'Lorem ipsum dolor sit amet Lorem ipsum dolor sit amet',
                label: {
                    text: 'Lorem ipsum dolor sit amet',
                    position: 'right',
                    class: 'text-uppercase font-weight-bold'
                }
            },
            ...commonProps
        }}
    >
        {Drupal.bind({})}
    </Story>
</Canvas>

## Drupal implementation example

```jsx

{% include '@bf-components/banner-advanced/twig/index.twig' with {
    title: 'Mobile discount of $10 with an Internet plan',
    description: 'Combine a Mobile plan to an Internet plan and get a $10 monthly discount.',
    icons: [
        {
            icon: 'mobility',
            label: 'Mobile plan'
        },
        {
            icon: 'bundle-1',
            label: 'Internet plan'
        }
    ]
} only %}

// Banner with switch
{% include '@bf-components/banner-advanced/twig/index.twig' with {
    title: 'Mobile discount of $10 with an Internet plan',
    description: 'Combine a Mobile plan to an Internet plan and get a $10 monthly discount.',
    icons: [
        {
            icon: 'mobility',
            label: 'Mobile plan'
        },
        {
            icon: 'bundle-1',
            label: 'Internet plan'
        }
    ],
    inputSwitch: {
        id: 'inputId',
        name: 'inputName',
        ariaLabel: 'Lorem ipsum dolor sit amet Lorem ipsum dolor sit amet',
        label: {
            text: 'Lorem ipsum dolor sit amet',
            position: 'right',
            class: 'text-uppercase font-weight-bold'
        }
    }
} only %}

// Banner with embedded footer
{% embed '@bf-components/banner-advanced/twig/index.twig' with {
    title: 'Mobile discount of $10 with an Internet plan',
    description: 'Combine a Mobile plan to an Internet plan and get a $10 monthly discount.',
    icons: [
        {
            icon: 'mobility',
            label: 'Mobile plan'
        },
        {
            icon: 'bundle-1',
            label: 'Internet plan'
        }
    ],
} only %}
    {% block footer %}
        // Insert html here.
    {% endblock %}
{% endembed %}
```

## Design Guidelines

<iframe width='100%' height='450' src='https://www.figma.com/embed?embed_host=share&url=https%3A%2F%2Fwww.figma.com%2Ffile%2F7bTmk29aCBl7rCFszffYPr%2FDS---Components-%26-styles%3Fnode-id%3D21784%3A284582' frameBorder='0' allowFullScreen></iframe>