...
import { OldHeaderComponent } from '../angular/header.component';

<Meta
    title="Components/__OLD - TO DELETE BEFORE FINAL COMMIT - Navigation Header"
    component={OldHeaderComponent}
    decorators={[moduleMetadata({ declarations: [OldHeaderComponent, TwigContainerComponent] })]}
    argTypes={{
        accountHello: {
...
```jsx
{% include '@bf-components/__old-navigation-header/twig/index.twig' with {
    hasTopBar: true,
    topBar: {
        items: [
...
<ArgsTable of={OldHeaderComponent} />
...
<Canvas withSource="none">
    <Story
        name="Drupal"
        args={{
            elementPath: 'components/__old-navigation-header/_doc/template.drupal',
            topBar_items,
