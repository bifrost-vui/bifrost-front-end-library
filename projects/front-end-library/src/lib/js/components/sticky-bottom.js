...
export let stickyBottomBar = {
    ...
    updateOffset() {
        if (
            this.bottomElemOffset !== this.registeredElemBottomOffset &&
            !this.elem.classList.contains('fixed-bottom')
        ) {
            this.registeredElemBottomOffset = this.bottomElemOffset;
        }
    },
    ...
    updateStickyNavState() {
        if (!this.isScrolling && !this.isDisabled) {
            ...
            if (
                window.scrollY + window.innerHeight < this.registeredElemBottomOffset &&
                (!this.elemTopTarget || this.bottomOffsetTopTarget < window.scrollY)
            ) {
